...
#######################
#######################################################################################################
# https://www.learncs.site/assets/files/RISCV_Calling_Convention-fb38c62ecbe1cc07ab5b25522d6192e4.pdf #
#######################################################################################################
...
inc_arr:
    ...
    sw s1, 8(sp)
    # END PROLOGUE
    mv s0, a0 # Copy start of array to saved register mv rd, rs
...
inc_arr_loop:
    ...
    # Also ask yourself this: why don't we need to preserve t1? temp...
    #
    addi sp, sp, -4
    sw t0, 0(sp)
    jal helper_fn
