...
map:
    ...
    lw t1, 0(s0)      # load the address of the array of current node into t1 #####1 在循环外面进行初始化
    lw t2, 4(s0)        # load the size of the node's array into t2    
    addi t1, t1, -4      

mapLoop:

    addi t1, t1, 4      # offset the array address by the count #####2 直接加t0会爆
    ...
    lw a0, 8(s0)        # load the address of the next node into a0 #####4
    mv a1, s1        # put the address of the function back into a1 to prepare for the recursion#####5 s1 points to a func, s0 point to a struct!
